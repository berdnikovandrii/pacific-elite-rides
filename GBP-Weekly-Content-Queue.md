# GBP Weekly Content Queue — Pacific Elite Rides

Running log of Google Business Profile post drafts and blog topic ideas, generated weekly. Check prior weeks before reusing an angle or topic.

---

## Week of 2026-07-27

### GBP Post Drafts (fill in bracketed details before posting)

**1. Airport Transfer**
> Flying out of SAN or landing at LAX this [day]? Pacific Elite Rides offers flat-rate airport transfers starting at $60 — no surge pricing, ever. We track your flight, so if it's delayed, we're still there. Just picked up a client from [pickup location] headed to [destination] in our Cadillac Escalade — smooth ride, on time, every time. Book online or call ahead for guaranteed availability. San Diego's flat-rate alternative to rideshare surge pricing.

**2. Sprinter Van / Group Event**
> Planning a [event type] with a group of 7-14 people? Our Mercedes Sprinter van comes with an upscale, party-bus-style interior — perfect for [event type] around San Diego. $160/hr, 4-hour minimum. This [day] we ran a group out to [pickup location] and back — everyone rode together, no separate cars, no parking hassle. Reach out to lock in your date.

**3. San Diego Comic-Con (event tie-in)**
> San Diego Comic-Con just wrapped downtown (July 22-26) and we were out running airport transfers and group rides all week. Picked up [number] of guests from [pickup location] headed to the Convention Center / [hotel name] this [day] — no surge pricing even with the city packed. If you're flying in or out of SAN for a big San Diego event, flat-rate booking beats fighting rideshare surge. Book ahead for your next convention or downtown event.

### Blog Topic Idea (queued — no blog section live yet, save for when built)

**"SAN Airport Transfer Flat Rate: What's Actually Included (No Surge, No Hidden Fees)"** — ties to the "SAN airport transfer flat rate" keyword from the Google Ads campaign. Angle: break down what the $60 flat rate covers (flight tracking, meet-and-greet, no surge multiplier) vs. what riders typically get surprised by with Uber Black surge pricing at SAN. Strong direct-booking angle since it's a head-to-head differentiator no competitor content addresses.

**Reminder:** No blog section exists on pacificeliterides.com yet — this topic is saved for whenever that gets built, not for immediate publishing.

---

## Week of 2026-08-03

### GBP Post Drafts (fill in bracketed details before posting)

**1. Zack Bryan Concert (event tie-in)**
> Zack Bryan played San Diego this week and we were running non-stop — pickups and drop-offs all night for concertgoers headed downtown. No surge pricing, no waiting in a rideshare queue after the show. We picked up [number] guests from [pickup location] and got them to [venue] and back safely in our [vehicle]. Got a concert, game, or big event coming up? Book your ride ahead of time and skip the post-show chaos.

**2. Padres Game (group/event tie-in)**
> Padres game [day] and Pacific Elite Rides was on it — shuttling fans to and from Petco Park all [afternoon/evening]. Whether it's a couple of friends or a full Sprinter van of [number], we get you there without the parking hassle or surge pricing spike. Ran a group from [pickup location] this week and had them home by [time]. Heading to the next home game? Reserve your ride in advance.

### Blog Topic Idea (queued — no blog section live yet, save for when built)

**"Sprinter Van San Diego: The Smart Way to Get a Group to Petco Park or a Concert Without Splitting Into 4 Ubers"** — ties to the "Sprinter van San Diego groups" keyword from the Google Ads campaign. Angle: use game-day and concert traffic/parking pain as the hook, position the Sprinter van's flat hourly rate ($160/hr, 4-hr min) against the cost and hassle of multiple rideshares plus stadium parking. Timely given this week's Padres and Zack Bryan rides.

---

## Week of 2026-08-10

### GBP Post Drafts (fill in bracketed details before posting)

**1. Corporate Ride**
> Business travel doesn't leave room for a late pickup. Pacific Elite Rides runs corporate car service across San Diego — airport runs, client meetings, downtown office transfers — in a clean, professional Suburban or Escalade, every time. This [day] we had a rider headed from [pickup location] to a meeting near [destination], on time with zero scrambling for parking. Set up standing account rides for your team or book one-off trips as needed.

**2. Airport Transfer**
> Heading to SAN or LAX for a business trip? Pacific Elite Rides keeps it simple: flat rate from $60, flight tracking built in, no surge pricing even when your flight lands late. This [day] we picked up a traveler from [pickup location] and had them at the terminal with time to spare — no app surge, no guessing on cost. Book ahead for guaranteed pickup, any day of the week.

### Blog Topic Idea (queued — no blog section live yet, save for when built)

**"Corporate Car Service San Diego: Why Businesses Are Ditching Rideshare for Airport and Client Transfers"** — ties to the "corporate car service San Diego" keyword from the Google Ads campaign. Angle: cost/reliability comparison for companies booking repeat trips (airport pickups, client meetings, office transfers) — flat-rate predictability and flight tracking vs. rideshare surge and driver cancellations, plus the option of a standing account for regular business travelers. Not yet used in this queue.

---

---

## Week of 2026-08-12 — BLOG LAUNCH

Blog is now LIVE at pacificeliterides.com/blog.html with two articles. GBP posts below link to them (new flow: blog on site → GBP post links back).

### GBP Post Drafts

**1. Flat rate article promo**
> Ever landed at SAN and watched the rideshare price double while you walked to baggage claim? We wrote up exactly what our $60 flat rate includes — flight tracking, meet & greet inside the terminal, 60 minutes of free wait time — and when rideshare actually beats us (yes, we're honest about it). Read the full breakdown on our blog: https://pacificeliterides.com/blog/san-airport-transfer-flat-rate-explained.html

**2. Sprinter group article promo**
> Taking 10+ people to a Padres game or concert? Four Ubers on surge pricing costs more than you think — and nobody arrives together. We broke down the real math of one Sprinter party bus vs. splitting into rideshares (spoiler: ~$51/person for the whole evening, LED lights and bar with ice included). Full article: https://pacificeliterides.com/blog/sprinter-van-group-petco-park-concerts.html

